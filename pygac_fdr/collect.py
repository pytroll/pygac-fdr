...
def collect_l1c_metadata(filenames, dbfile, if_exists):
    """Collect metadata from l1c files and save to database."""
    collector = L1cMetadataCollector()
    mda = collector.collect_metadata(filenames)
    enhancer = L1cMetadataEnhancer()
    mda = enhancer.update_metadata(mda)
    save_metadata_to_database(mda, dbfile, if_exists)


class L1cMetadataCollector:
...
class L1cMetadataEnhancer:
    """Enhance metadata collected from level 1c files."""
    ...
    def update_metadata(self, metadata: pd.DataFrame) -> pd.DataFrame:
        """Enhance metadata collected from level 1c files.

        Adds new columns including

        - global quality flag
        - equator crossing time
        - overlap
        """
        self._sort_by_ascending_time(metadata)
        metadata = self._set_global_quality_flag(metadata)
        metadata = self._calc_overlap(metadata)
        metadata = self._update_index(metadata)
        return metadata
